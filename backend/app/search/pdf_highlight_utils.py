...
async def get_highlighted_pdf(
    ...
    if page_keywords:
        ...
        cache_key = f"{pdf_url}_paged_{hashlib.md5(page_keywords_str.encode()).hexdigest()}"
    else:
    ...
    if os.path.exists(highlighted_path):
        ...
        return f"/highlighted_pdfs/{pdf_filename}" if not bucket_url_prefix else f"{bucket_url_prefix}/{pdf_filename}"
    ...
    task = asyncio.create_task(
        _process_and_highlight_pdf(pdf_url, search_term, highlighted_path, page_keywords)
    )
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to create highlighted PDF: {str(e)}")
...
async def _process_and_highlight_pdf(
    ...
    try:
        ...
        if page_keywords and len(page_keywords) > 0:
            logger.info(f"Adding page-specific highlights to PDF")
            _add_page_specific_highlights(temp_path, output_path, page_keywords)
        else:
            # Fall back to using the search term for all pages
            logger.info(f"Adding highlights for term '{search_term}' to PDF (all pages)")
        ...
        for page_num_str, keywords in page_keywords.items():
            try:
                ...
                if page_num < 0 or page_num >= len(doc):
                    logger.warning(f"Page number {page_num+1} is out of bounds for PDF with {len(doc)} pages")
                    continue
                ...
                if page_highlights_count > 0:
                    logger.info(f"Page {page_num+1}: Added {page_highlights_count} highlights for keywords {keywords}")
...
def _add_highlights(input_path: str, output_path: str, search_terms: str) -> None:
    ...
    if "," in search_terms:
        ...
        logger.info(f"Processing PDF {input_path} to highlight multiple terms: {terms_list}")
    else:
    ...
    try:
        ...
        for page_num, page in enumerate(doc):
            ...
            if page_highlights_count > 0:
                logger.info(f"Page {page_num+1}: Added {page_highlights_count} highlights")
...
<body>
    ...
    logger.info(f"Mounted highlighted PDFs directory at /highlighted_pdfs")